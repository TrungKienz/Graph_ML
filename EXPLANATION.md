...
Điểm **hội tụ** của toàn bộ pipeline huấn luyện — nhận cả 4 loss, xuất tín hiệu gradient duy nhất.
...
[Khối 2] Backbone ×1  [Khối 9] Nhánh B → [Khối 2] Backbone ×2
   │                      │
   ▼                      ▼
user_emb, item_emb    2 bộ embedding
   │                      │
   ▼                      ▼
   ...
   └──────────┬───────────┘
       ...
       [Khối 12] Total Loss
          ...
          Gradient
              │
              ▼
   Cập nhật Embedding (quay lại Khối 2, lặp lại)
```
...
Điểm thiết kế quan trọng nhất là **tách kiến trúc thành 2 nhánh song song, dùng chung một Backbone**. Nhánh chính (Phần III) giữ nguyên y hệt LightGCN gốc, chịu trách nhiệm tạo embedding dùng để dự đoán và suy luận thật. Nhánh phụ (Phần IV) hoàn toàn tách biệt — tạo 2 phiên bản đồ thị "nhiễu có chủ đích" (làm yếu tín hiệu item phổ biến), dùng Contrastive Learning để embedding ổn định hơn — nhưng **không bao giờ** được dùng để đưa ra gợi ý cuối cùng.
